...
GraphDataSet::LabelInfo::LabelInfo(int l, int f, int o){
    label=l;
    frequency=f;
    order=o;
...
bool GraphDataSet::LabelInfo::operator<(GraphDataSet::LabelInfo &b){
    return this->frequency<b.frequency?true:false;
}

bool GraphDataSet::LabelInfo::compareByLabel(const GraphDataSet::LabelInfo &a, const GraphDataSet::LabelInfo &b){
    return a.label<b.label?true:false;
}

GraphDataSet::LabelInfoSet::LabelInfoSet(vector<int> &labels){
    for(unsigned i=0;i < labels.size();i++){
        this->push_back(LabelInfo(i,labels[i],-1));
    }
}

int GraphDataSet::LabelInfoSet::setOrder(){
    unsigned j=0;
...
void GraphDataSet::LabelInfoSet::filter(int min_sup){
    for(unsigned i=0;i < (*this).size();i++){
